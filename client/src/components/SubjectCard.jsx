...
const SubjectCard = ({ subject, onView, onEdit, onDelete }) => {
    ...
    return (
        <div
            className="bg-[#10141e] p-4 rounded-lg shadow-md relative cursor-pointer border border-gray-600"
            onClick={() => onView(subject._id)}
        >
            <h3 className="text-lg font-semibold mb-2 text-white">{subject.title}</h3>

            {subject.description && <p className="text-gray-300 mb-4">{subject.description}</p>}
